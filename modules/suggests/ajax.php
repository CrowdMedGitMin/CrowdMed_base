...
if($_GET['action'] == 'grab'){
	...
	$sql = "
		SELECT s.id, s.suggestion, if (v.ip IS NULL,0,1) AS have_voted
	...
	try {
		...
		$stmt->bind_param('siis', $data['ip'],$_GET['market'],$_GET['concept'],$_GET['culture']);
		$stmt->execute();
	...
	$sql = "INSERT INTO suggestions SET suggestion = ?, market_id = ?, concept_id = ?, culture = ?";
	try {
		$stmt = $dbo->prepare($sql);
		$stmt->bind_param('siis', $_GET['content'], $_GET['market'], $_GET['concept'], $_GET['culture']);
		$stmt->execute();
		$stmt->close();
	} catch( Exception $e ){
		die( $e->getMessage() );
